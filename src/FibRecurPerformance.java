...
public class FibRecurPerformance {
    ...
    static int MAXINPUTSIZE = 50;  // time, processor speed, and available memory
    ...
    static void runFullExperiment(String resultsFileName) {
        ...
        for (int inputSize = MININPUTSIZE; inputSize <= MAXINPUTSIZE; inputSize++) {
            ...
            for (long trial = 0; trial < numberOfTrials; trial++) {
                ...
                long FibTest = FibRecur(inputSize);
    ...
    // 'classic' Fibonacci sequence algorithm using recursion
    static long FibRecur(long x)
    {
        // handle 'base case'
        if (x <= 1)
            return x;
        // recursively call previous two values for calculation
